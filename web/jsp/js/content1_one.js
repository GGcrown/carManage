...
function showInsuranceList(insurances) {
	for (var i = 0; i < insurances.length; i++) {
		...
		var $a = $("<a insureId='" + insurances[i].insureId + "'>详细信息</a>");
		$td.append($a);
		tr.append($td);

		$td = $("<td align='center'></td>");
		var $a2 = $("<a insureId='" + insurances[i].insureId + "'>更改信息</a>");
		var $a3 = $("<a insureId='" + insurances[i].insureId + "'>删除</a>");
		$td.append($a3);
		$td.append($a2);
		tr.append($td);
		...
		$a3.click(function() {
			swal({
				...
				if (isConfirm == true) {
					var insureId = $(this).attr("insureId");
					// TODO
					$.post("insurance/.action?insureId=" + cid, {}, function(data) {
						if (data > 0) {
							swal("删除成功！", "", "success");
							filpPage($("li>a[index=1]").text() - 1);
							$("#animateDIV").slideUp();
